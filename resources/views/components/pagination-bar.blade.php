{{--
    ...
    Props:
        ...
        wire      — pass true for in-memory paginators; uses wire:click instead of href + wire:navigate

    Usage:
        <x-pagination-bar :paginator="$paginator" :per-page="$perPage" />
        <x-pagination-bar :paginator="$paginator" :per-page="$perPage" :wire="true" :options="[10, 20, 50, 0]" />
--}}
@props(['paginator', 'perPage', 'method' => 'setPerPage', 'options' => [20, 40, 80, 100, 0], 'wire' => false])

<div class="mt-6">
    {{-- Mobile: page controls row, then showing/per-page row --}}
    <div class="sm:hidden flex flex-col gap-3">
        <div class="flex justify-center">
            <x-pagination-selector :paginator="$paginator" :wire="$wire" />
        </div>
