...
| `WaterServicesData` | `WaterServicesService` | One NWIS site × parameter time series. Holds latest reading value + datetime. `toArray(string $timezone)` includes formatted time and provisional flag. |
| `FloodAlertData` | `NWSAlertsService` | One NWS active alert. Has `FLOOD_EVENT_TYPES` constant. `toArray(string $timezone)` includes severity badge styles, formatted times, and geometry for map rendering. |